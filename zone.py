...
DB_FILE = 'community_fresh.db'  # New name to avoid any old locks/corruption

# Delete old file completely to start over every run (remove this line later in production!)
...
# Create tables DIRECTLY – no IF NOT EXISTS, no condition
...
# ===================== NEW: REELS TABLE (added exactly as requested) =====================
...
# Update get_db to use the new file
def get_db():
    conn = sqlite3.connect(DB_FILE)
...
def like(news_id, action):
    if 'user_id' not in session:
        return jsonify({
            "success": False,
            "message": "Please log in to like/dislike."
        })
    ...
    try:
        ...
        pass  # already voted (unique constraint)
...
def news_detail(news_id):
    conn = get_db()
    c = conn.cursor()
    c.row_factory = sqlite3.Row   # so {{ post.heading }} works like in your other templates
    ...
    # Get current like/dislike counts (so voting works on the detail page too)
    c.execute("SELECT COUNT(*) FROM likes WHERE news_id = ? AND type = 'like'", (news_id,))
    likes = c.fetchone()[0]
...
# ===================== NEW: REEL POSTING (full functionality added – same pattern as /news) =====================
...
def reel():
    ...
    conn.close()
    return render_template('reel.html', reels_list=all_reels)

# ===================== NEW: PROFILE FUNCTIONALITY (shows YOUR news + reels exactly like Facebook profile) =====================
@app.route('/profile')
def profile():
    if 'user_id' not in session:
        return redirect(url_for('log'))
    ...
    # Your news posts (exactly as requested)
    user_news = c.execute("""
        SELECT n.*, u.fname || ' ' || u.lname AS author 
        FROM news n JOIN users u ON n.user_id = u.id 
        WHERE n.user_id = ? 
    ...
    # Your reels (added for completeness)
